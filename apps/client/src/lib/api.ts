...
export const apiClient = {
  ...
  auth: {
    ...
    signOut: () => api.post<void>(API_ENDPOINTS.AUTH.SIGN_OUT),

    session: () => api.get<AuthResponse>(API_ENDPOINTS.AUTH.SESSION),
  },
  ...
  jobs: {
    ...
    get: (id: string) => api.get<{ job: Job }>(API_ENDPOINTS.JOBS.GET(id)),

    action: (id: string, action: "retry" | "remove" | "promote") =>
      api.post<JobActionResponse>(API_ENDPOINTS.JOBS.ACTION(id), {
        action,
        jobId: id,
      }),

    stats: () => api.get<QueueStats>(API_ENDPOINTS.JOBS.STATS),
  },
};
